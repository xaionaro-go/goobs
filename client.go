...
func (c *Client) connect() (err error) {
	...
	go func() {
		...
		// we write to IncomingResponses only from two places:
		// * c.handleOpcodes
		// * c.client.SendRequest
		// thus the `close` should happen only after both would not
		// write into this channel anymore, so we put it here:
